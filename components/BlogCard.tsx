...
const BlogCard = (props: {
    ...
    return (
        <>
            <Link href={props.link.startsWith("https") ? props.link : props.link === "bucketlist" ? "/bucketlist" : `/blog/${props.link}`} target="_blank" rel="noreferrer"><div className='hover:text-teal underline hover:cursor-pointer text-xl mt-4 md:hidden block'>{props.name}</div></Link>
            <div className='mt-7 md:block hidden max-w-3-1xl'>
                <Link href={props.link.startsWith("https") ? props.link : props.link === "bucketlist" ? "/bucketlist" : `/blog/${props.link}`} target="_blank" rel="noreferrer">
